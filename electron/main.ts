import { app, BrowserWindow, nativeImage, dialog, session, utilityProcess, ipcMain, shell, Tray, Menu, safeStorage } from 'electron';
import path from 'path';
import { execFileSync, spawn, ChildProcess } from 'child_process';
import fs from 'fs';
import net from 'net';
import os from 'os';
import { FileSecureStore } from '../src/lib/secure-store';
import type {
  GitHubAuthSession,
...
let githubCloneProcess: ChildProcess | null = null;

const GITHUB_SESSION_KEY = 'github.device_flow.session';
...
function getSecureStore(): FileSecureStore {
  return new FileSecureStore(
    path.join(app.getPath('userData'), 'secure-store.json'),
    safeStorage,
  );
}

function isGitHubHttpsRepository(repositoryUrl: string): boolean {
  try {
...
function parseCloneProgress(message: string): GitHubCloneProgressEvent {
  const trimmed = message.trim();
  const percentMatch = trimmed.match(/(\d{1,3})%/);
  return {
    message: trimmed,
    phase: 'cloning',
    percent: percentMatch ? Number.parseInt(percentMatch[1], 10) : undefined,
  };
}

function sendCloneProgress(event: GitHubCloneProgressEvent): void {
  mainWindow?.webContents.send('github:clone-progress', event);
}

async function readGitHubSession(): Promise<GitHubAuthSession | null> {
  if (!safeStorage.isEncryptionAvailable()) {
    return null;
  }
  try {
    return await getSecureStore().getJson<GitHubAuthSession>(GITHUB_SESSION_KEY);
  } catch (error) {
    console.warn('[github-auth] Failed to read secure session:', error);
    return null;
  }
}

async function writeGitHubSession(sessionData: GitHubAuthSession): Promise<void> {
  await getSecureStore().setJson(GITHUB_SESSION_KEY, sessionData);
}

async function clearGitHubSession(): Promise<void> {
  await getSecureStore().delete(GITHUB_SESSION_KEY);
}

function toGitHubAuthSummary(sessionData: GitHubAuthSession | null): GitHubAuthSessionSummary {
  return {
    supported: true,
    configured: Boolean(getGitHubClientId()),
    secure_storage_available: safeStorage.isEncryptionAvailable(),
    connected: Boolean(sessionData?.access_token),
    scope: sessionData?.scope,
    acquired_at: sessionData?.acquired_at,
    user: sessionData?.user ?? null,
  };
}

async function cloneRepositoryWithGitHubAuth(
  ...
  if (!nextUrl || !nextDestination) {
    return { success: false, error: 'repositoryUrl and destination are required' };
  }

  if (!path.isAbsolute(nextDestination)) {
    return { success: false, error: 'Destination must be an absolute local path' };
  }

  if (githubCloneProcess) {
    return { success: false, error: 'Another GitHub clone is already running' };
  }

  const parentDir = path.dirname(nextDestination);
  fs.mkdirSync(parentDir, { recursive: true });

  const storedSession = await readGitHubSession();
  const env: Record<string, string> = {
    ...sanitizedProcessEnv(),
    ...userShellEnv,
    HOME: os.homedir(),
    USERPROFILE: os.homedir(),
    PATH: getExpandedShellPath(),
    GIT_TERMINAL_PROMPT: '0',
  };

  if (storedSession?.access_token && isGitHubHttpsRepository(nextUrl)) {
    const origin = new URL(nextUrl).origin;
    const basicAuth = Buffer.from(`x-access-token:${storedSession.access_token}`).toString('base64');
  ...
  return await new Promise<GitHubCloneResult>((resolve) => {
    let lastMessage = '';
    const child = spawn('git', ['clone', '--progress', nextUrl, nextDestination], {
      cwd: parentDir,
      env,
      stdio: ['ignore', 'pipe', 'pipe'],
    });
    githubCloneProcess = child;
    ...
    const handleChunk = (chunk: Buffer) => {
      for (const line of chunk.toString().split(/\r?\n/)) {
        ...
        if (!trimmed) continue;
        lastMessage = trimmed;
        sendCloneProgress(parseCloneProgress(trimmed));
      }
    };
    ...
    child.on('exit', (code) => {
      ...
      if (code === 0) {
        ...
        resolve({ success: true, destination: nextDestination });
        return;
      }

      const error = lastMessage || `git clone failed with exit code ${code ?? 'unknown'}`;
      sendCloneProgress({
        phase: 'error',
        message: error,
        error,
  ...
  const contextMenu = Menu.buildFromTemplate([
    {
      ...
      click: () => {
        if (BrowserWindow.getAllWindows().length === 0) {
          createWindow(`http://localhost:${serverPort || 3000}`);
        } else {
          mainWindow?.focus();
        }
      },
...
app.whenReady().then(async () => {
  ...
  ipcMain.handle('github:get-session', async () => {
    const sessionData = await readGitHubSession();
    return toGitHubAuthSummary(sessionData);
  });

  ipcMain.handle('github:store-session', async (_event: Electron.IpcMainInvokeEvent, sessionData: GitHubAuthSession) => {
    await writeGitHubSession(sessionData);
    return toGitHubAuthSummary(await readGitHubSession());
  });

  ipcMain.handle('github:clear-session', async () => {
    await clearGitHubSession();
    return toGitHubAuthSummary(null);
  });

  ipcMain.handle(
    'github:clone-repository',
    async (_event: Electron.IpcMainInvokeEvent, input: { repositoryUrl: string; destination: string }) => {
      return cloneRepositoryWithGitHubAuth(input.repositoryUrl, input.destination);
    },
  );
  ...
  try {
    ...
    if (isDev) {
      port = 3000;
      console.log(`Dev mode: connecting to http://localhost:${port}`);
      serverPort = port;
