...
## Remote Transport 子系统

`src/lib/transport/` — 统一的命令执行和文件操作抽象，支持两种传输模式：

- **local** — 本地直接执行（默认）
- **ssh_direct** — 纯 SSH 远程执行，无本地镜像

**核心组件：**
- `types.ts` — `RuntimeTransport` 接口、配置类型、事件类型
- `local-transport.ts` / `ssh-direct-transport.ts` — 两种实现
- `transport-factory.ts` — 工厂函数（`createTransport`, `createTransportForSession`）
- `src/lib/remote-health-monitor.ts` — SSH 连接心跳检测 + 指数退避自动重连
- `src/components/remote/ConnectionHealthIndicator.tsx` — 连接状态指示器
- `src/components/remote/ConnectionReconnectBanner.tsx` — 断连警告横幅

详细文档：`docs/handover/transport-layer.md`
...
| Transport | `src/lib/transport/` | 如涉及远程执行或新传输模式 |
