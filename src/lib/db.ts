...
import fs from 'fs';
import os from 'os';
import type { ChatSession, Message, SettingsMap, TaskItem, TaskStatus, ApiProvider, CreateProviderRequest, UpdateProviderRequest, MediaJob, MediaJobStatus, MediaJobItem, MediaJobItemStatus, MediaContextEvent, BatchConfig, ShellTranscriptEntry } from '@/types';
...
import { isRemoteShellState } from '@/lib/shell-transcript';
...
function initDb(db: Database.Database): void {
  db.exec(`
    CREATE TABLE IF NOT EXISTS chat_sessions (
      ...
      workspace_transport TEXT NOT NULL DEFAULT 'local',
      remote_connection_id TEXT NOT NULL DEFAULT '',
      remote_path TEXT NOT NULL DEFAULT '',
      sdk_session_id TEXT NOT NULL DEFAULT '',
      engine_type TEXT NOT NULL DEFAULT 'claude',
    ...
    CREATE TABLE IF NOT EXISTS terminal_transcript_entries (
      id TEXT PRIMARY KEY,
      session_id TEXT NOT NULL,
      kind TEXT NOT NULL CHECK(kind IN ('command', 'output')),
    ...
    CREATE INDEX IF NOT EXISTS idx_terminal_transcript_session_created_at ON terminal_transcript_entries(session_id, created_at);
...
function migrateDb(db: Database.Database): void {
  ...
  if (!colNames.includes('reasoning_effort')) {
    db.exec("ALTER TABLE chat_sessions ADD COLUMN reasoning_effort TEXT NOT NULL DEFAULT ''");
  }
  if (!colNames.includes('system_prompt')) {
    db.exec("ALTER TABLE chat_sessions ADD COLUMN system_prompt TEXT NOT NULL DEFAULT ''");
  }
  if (!colNames.includes('workspace_transport')) {
    db.exec("ALTER TABLE chat_sessions ADD COLUMN workspace_transport TEXT NOT NULL DEFAULT 'local'");
  }
  if (!colNames.includes('remote_connection_id')) {
    db.exec("ALTER TABLE chat_sessions ADD COLUMN remote_connection_id TEXT NOT NULL DEFAULT ''");
  }
  if (!colNames.includes('remote_path')) {
    db.exec("ALTER TABLE chat_sessions ADD COLUMN remote_path TEXT NOT NULL DEFAULT ''");
  }
  if (!colNames.includes('sdk_session_id')) {
    db.exec("ALTER TABLE chat_sessions ADD COLUMN sdk_session_id TEXT NOT NULL DEFAULT ''");
  }
  if (!colNames.includes('engine_type')) {
    db.exec("ALTER TABLE chat_sessions ADD COLUMN engine_type TEXT NOT NULL DEFAULT 'claude'");
  }
  if (!colNames.includes('engine_session_id')) {
    db.exec("ALTER TABLE chat_sessions ADD COLUMN engine_session_id TEXT NOT NULL DEFAULT ''");
  }
  // Compatibility bridge: keep sdk_session_id and engine_session_id in sync during migration.
  db.exec("UPDATE chat_sessions SET engine_session_id = sdk_session_id WHERE engine_session_id = '' AND sdk_session_id != ''");
  db.exec("UPDATE chat_sessions SET sdk_session_id = engine_session_id WHERE sdk_session_id = '' AND engine_session_id != ''");
  ...
  const bindingColNames = bindingCols.map(c => c.name);
  if (bindingColNames.length > 0 && !bindingColNames.includes('engine_type')) {
    db.exec("ALTER TABLE channel_bindings ADD COLUMN engine_type TEXT NOT NULL DEFAULT 'claude'");
  }
  if (bindingColNames.length > 0 && !bindingColNames.includes('engine_session_id')) {
    db.exec("ALTER TABLE channel_bindings ADD COLUMN engine_session_id TEXT NOT NULL DEFAULT ''");
  }
  // Compatibility bridge: keep sdk_session_id and engine_session_id in sync during migration.
  db.exec("UPDATE channel_bindings SET engine_session_id = sdk_session_id WHERE engine_session_id = '' AND sdk_session_id != ''");
  db.exec("UPDATE channel_bindings SET sdk_session_id = engine_session_id WHERE sdk_session_id = '' AND engine_session_id != ''");
...
function normalizeEngineSessionId(row: { engine_session_id?: string; sdk_session_id?: string }): string {
  return row.engine_session_id || row.sdk_session_id || '';
}

function mapChatSession(row: ChatSessionRow): ChatSession {
  const engineSessionId = normalizeEngineSessionId(row);
  // Map workspace_transport with backward compatibility
  const transport = row.workspace_transport;
  const workspace_transport = transport === 'ssh_direct' ? 'ssh_direct' : 'local';
  return {
    ...row,
    workspace_transport,
    remote_connection_id: row.remote_connection_id || '',
    remote_path: row.remote_path || '',
    engine_type: row.engine_type || 'claude',
    engine_session_id: engineSessionId,
    // Compatibility read path: prefer new engine_session_id when available.
...
export function getAllSessions(): ChatSession[] {
  const db = getDb();
  const rows = db.prepare('SELECT * FROM chat_sessions ORDER BY updated_at DESC').all() as ChatSessionRow[];
...
export function createSession(
  ...
  const resolvedWorkspaceTransport = workspaceTransport === 'ssh_direct' ? 'ssh_direct' : 'local';
  const resolvedRemoteConnectionId = remoteConnectionId || '';
  const resolvedRemotePath = remotePath || '';
  const projectName = resolvedWorkspaceTransport === 'ssh_direct'
    ? path.posix.basename(resolvedRemotePath || '/') || 'remote'
    : path.basename(wd);
  const resolvedEngineType = normalizeEngineType(engineType || 'claude');
  const resolvedEngineSessionId = engineSessionId || '';
  ...
  const resolvedReasoningEffort = resolvedEngineType === 'codex'
    ? (
        normalizeReasoningEffort(reasoningEffort)
        || getCliDefaultsForEngine('codex').reasoningEffort
      )
    : '';
  const resolvedProviderId = providerId !== undefined
    ? providerId
    : getCliDefaultsForEngine(resolvedEngineType).providerId;

  db.prepare(
    'INSERT INTO chat_sessions (id, title, created_at, updated_at, model, reasoning_effort, system_prompt, working_directory, workspace_transport, remote_connection_id, remote_path, sdk_session_id, project_name, status, mode, sdk_cwd, provider_id, engine_type, engine_session_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
  ).run(
    id,
    title || 'New Chat',
    now,
    ...
    systemPrompt || '',
    wd,
    resolvedWorkspaceTransport,
    resolvedRemoteConnectionId,
    resolvedRemotePath,
    resolvedEngineSessionId,
    projectName,
    'active',
    mode || 'code',
    wd,
...
export function updateSdkSessionId(id: string, sdkSessionId: string): void {
  const db = getDb();
  db.prepare('UPDATE chat_sessions SET sdk_session_id = ?, engine_session_id = ? WHERE id = ?').run(sdkSessionId, sdkSessionId, id);
}
...
export function updateSessionReasoningEffort(id: string, reasoningEffort: string): void {
  const db = getDb();
  db.prepare('UPDATE chat_sessions SET reasoning_effort = ? WHERE id = ?').run(reasoningEffort, id);
}
...
export function updateSessionEngineType(id: string, engineType: string): void {
  const db = getDb();
  db.prepare('UPDATE chat_sessions SET engine_type = ? WHERE id = ?').run(engineType, id);
}
...
function mapShellTranscriptEntry(row: ShellTranscriptEntryRow): ShellTranscriptEntry {
  return {
    id: row.id,
    ...
    state: isRemoteShellState(row.state) ? row.state : null,
...
export function getShellTranscriptEntries(
  sessionId: string,
  options?: { limit?: number; beforeRowId?: number },
): { entries: ShellTranscriptEntry[]; hasMore: boolean } {
  const db = getDb();

  const limit = options?.limit ?? 100;
  const beforeRowId = options?.beforeRowId;

  let rows: ShellTranscriptEntryRow[];
  if (beforeRowId) {
    rows = db.prepare(
      'SELECT *, rowid as _rowid FROM terminal_transcript_entries WHERE session_id = ? AND rowid < ? ORDER BY rowid DESC LIMIT ?'
    ).all(sessionId, beforeRowId, limit + 1) as ShellTranscriptEntryRow[];
  } else {
    rows = db.prepare(
      'SELECT *, rowid as _rowid FROM terminal_transcript_entries WHERE session_id = ? ORDER BY rowid DESC LIMIT ?'
    ).all(sessionId, limit + 1) as ShellTranscriptEntryRow[];
  }
  ...
  rows.reverse();
  return {
    entries: rows.map(mapShellTranscriptEntry),
    hasMore,
  };
}

export function addShellTranscriptCommand(
  sessionId: string,
  command: string,
  ...
  const now = new Date().toISOString().replace('T', ' ').split('.')[0];
  db.prepare(`
    INSERT INTO terminal_transcript_entries (
      id, session_id, kind, command, remote_path, started_at, state, exit_code, signal, status, truncated, output, created_at, updated_at
    ) VALUES (?, ?, 'command', ?, ?, '', '', NULL, '', '', 0, '', ?, ?)
  `).run(id, sessionId, command, remotePath || '', now, now);

  updateSessionTimestamp(sessionId);

  return mapShellTranscriptEntry(
    db.prepare('SELECT *, rowid as _rowid FROM terminal_transcript_entries WHERE id = ?').get(id) as ShellTranscriptEntryRow,
  );
}

type UpsertShellTranscriptOutputInput = {
  command: string | null;
  remotePath: string | null;
...
export function addShellTranscriptOutput(
  sessionId: string,
  input: UpsertShellTranscriptOutputInput,
): ShellTranscriptEntry {
  const db = getDb();
...
export function updateShellTranscriptOutput(
  entryId: string,
  input: UpsertShellTranscriptOutputInput,
): ShellTranscriptEntry | undefined {
  const db = getDb();
  const now = new Date().toISOString().replace('T', ' ').split('.')[0];
  const result = db.prepare(`
    UPDATE terminal_transcript_entries
    SET command = ?, remote_path = ?, started_at = ?, state = ?, exit_code = ?, signal = ?, status = ?, truncated = ?, output = ?, updated_at = ?
    WHERE id = ? AND kind = 'output'
  `).run(
    input.command || '',
    input.remotePath || '',
    input.startedAt || '',
  ...
  const row = db.prepare('SELECT session_id FROM terminal_transcript_entries WHERE id = ?').get(entryId) as { session_id: string } | undefined;
  if (row) {
    updateSessionTimestamp(row.session_id);
  }

  return mapShellTranscriptEntry(
    db.prepare('SELECT *, rowid as _rowid FROM terminal_transcript_entries WHERE id = ?').get(entryId) as ShellTranscriptEntryRow,
  );
}


export function cloneSessionConversation(sourceSessionId: string, targetSessionId: string): number {
  const db = getDb();

  let clonedMessageCount = 0;

  const copyConversation = db.transaction(() => {
    const sourceMessages = db.prepare(
      'SELECT role, content, created_at, token_usage FROM messages WHERE session_id = ? ORDER BY rowid ASC'
    ...
    clonedMessageCount = sourceMessages.length;
    ...
    const sourceTasks = db.prepare(
      'SELECT title, status, description, source, sort_order, created_at, updated_at FROM tasks WHERE session_id = ? ORDER BY sort_order ASC, created_at ASC'
    ).all(sourceSessionId) as Array<{
      title: string;
      status: TaskStatus;
    ...
    sourceTasks.forEach((task, index) => {
      const taskId = task.source === 'sdk'
        ? 'sdk-' + targetSessionId + '-' + index
        : crypto.randomBytes(16).toString('hex');
      ...
      insertTask.run(
        ...
        task.description,
        task.source,
        typeof task.sort_order === 'number' ? task.sort_order : index,
        task.created_at,
        task.updated_at,
      );
    });
...
export function clearSessionMessages(sessionId: string): void {
  ...
  db.prepare('UPDATE chat_sessions SET sdk_session_id = ?, engine_session_id = ? WHERE id = ?').run('', '', sessionId);
}
...
export function getTokenUsageStats(days: number = 30, transport?: 'local' | 'ssh_direct', connectionId?: string): {
  summary: {
  ...
  if (transport) {
    sessionFilters.push(`cs.workspace_transport = '${transport}'`);
  }
  if (connectionId) {
    sessionFilters.push(`cs.remote_connection_id = '${connectionId.replace(/'/g, "''")}'`);
  }
  const transportFilter = sessionFilters.length > 0
    ? `AND EXISTS (SELECT 1 FROM chat_sessions cs WHERE cs.id = m.session_id AND ${sessionFilters.join(' AND ')})`
    : '';
...
function mapChannelBinding(row: ChannelBindingRow): ChannelBinding {
  const engineSessionId = normalizeEngineSessionId(row);
  return {
    id: row.id,
    ...
    engineType: row.engine_type || 'claude',
    engineSessionId,
    // Compatibility read path: prefer engine_session_id when available.
...
export function upsertChannelBinding(params: {
  ...
  const resolvedEngineType = normalizeEngineType(params.engineType ?? existing?.engineType ?? 'claude');
  const resolvedEngineSessionId =
    params.engineSessionId
    ?? params.sdkSessionId
  ...
  if (existing) {
    db.prepare(
      `UPDATE channel_bindings SET codepilot_session_id = ?, sdk_session_id = ?, engine_type = ?, engine_session_id = ?, working_directory = ?, model = ?, mode = ?, updated_at = ?
    ...
    const id = crypto.randomBytes(16).toString('hex');
    db.prepare(
      `INSERT INTO channel_bindings (id, channel_type, chat_id, codepilot_session_id, sdk_session_id, engine_type, engine_session_id, working_directory, model, mode, active, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?)`
    ).run(
      id,
      params.channelType,
...
export function updateChannelBinding(
  ...
  const engineSessionId = updates.engineSessionId ?? updates.sdkSessionId;

  if (updates.engineType !== undefined) { sets.push('engine_type = ?'); values.push(updates.engineType); }
  if (engineSessionId !== undefined) {
    sets.push('engine_session_id = ?');
    values.push(engineSessionId);
...
function registerShutdownHandlers(): void {
  ...
  const shutdown = (signal: string, options?: { silent?: boolean }) => {
  ...
  process.on('exit', () => shutdown('exit', { silent: true }));
