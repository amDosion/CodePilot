...
async function handleMessage(
  ...
  try {
    ...
    // Use text or empty string for image-only messages (prompt is still required by engine stream input)
    ...
    // Persist the actual engine session/thread ID for future resume.
    ...
    if (binding.id) {
      try {
        const nextEngineSessionId = result.engineSessionId || result.sdkSessionId;
        const existingEngineSessionId = binding.engineSessionId || binding.sdkSessionId;
        if (nextEngineSessionId) {
          updateChannelBinding(binding.id, { engineSessionId: nextEngineSessionId });
        } else if (result.hasError && existingEngineSessionId) {
          updateChannelBinding(binding.id, { engineSessionId: '' });
        }
      } catch { /* best effort */ }
    }
  } finally {
