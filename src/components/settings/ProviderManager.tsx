...
interface RuntimeStatusResponse {
  connected: boolean;
  engines: Record<RuntimeEngine, RuntimeEngineStatus>;
}

const EMPTY_RUNTIME_STATUS: RuntimeStatusResponse = {
  connected: false,
  engines: {
    claude: {
      available: false,
...
export function ProviderManager({ workspaceMode = 'local', remoteConnectionId = '' }: ProviderManagerProps) {
  const isRemote = workspaceMode === 'remote' && !!remoteConnectionId;
  const preferenceTarget = buildEnginePreferenceTarget(workspaceMode, remoteConnectionId);
  ...
  const fetchRuntimeStatus = useCallback(async () => {
    try {
      if (isRemote) {
        const res = await fetch("/api/remote/runtime-status", {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ connection_id: remoteConnectionId }),
        });
        if (!res.ok) throw new Error("Failed to load remote runtime status");
  ...
  const fetchProviders = useCallback(async () => {
    try {
      setError(null);
      if (isRemote) {
        const res = await fetch("/api/remote/providers", {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ connection_id: remoteConnectionId }),
        });
        if (!res.ok) throw new Error("Failed to load remote providers");
        const data = await res.json();
  ...
  useEffect(() => {
    const syncPreferences = () => {
      const nextEngine = readActiveEngine(preferenceTarget);
      const nextPreferences = readEnginePreferences(nextEngine, preferenceTarget);
      setActiveEngine(nextEngine);
      setActiveProviderId(nextPreferences.providerId || '');
      void fetchRuntimeStatus();
    };

    syncPreferences();
    window.addEventListener("engine-changed", syncPreferences);
    window.addEventListener("provider-changed", syncPreferences);
    window.addEventListener("session-updated", syncPreferences as EventListener);
    window.addEventListener("storage", syncPreferences);
    window.addEventListener("focus", syncPreferences);
    return () => {
      window.removeEventListener("engine-changed", syncPreferences);
      window.removeEventListener("provider-changed", syncPreferences);
      window.removeEventListener("session-updated", syncPreferences as EventListener);
      window.removeEventListener("storage", syncPreferences);
      window.removeEventListener("focus", syncPreferences);
    };
  }, [fetchRuntimeStatus, preferenceTarget]);

  const handleEngineSelect = useCallback((engine: RuntimeEngine) => {
    const nextPreferences = readEnginePreferences(engine, preferenceTarget);
    setActiveEngine(engine);
    setActiveProviderId(nextPreferences.providerId || '');
    persistEnginePreferences(engine, {}, preferenceTarget);
  ...
  const selectedProvider = sorted.find((provider) => provider.id === activeProviderId) || null;
  const selectedProviderLabel = activeProviderId === 'env'
    ? t('provider.environment')
  ...
  const runtimeProviders = ([
    {
      ...
      name: "Claude Code",
      icon: <Anthropic size={18} />,
      description: isRemote ? t("provider.runtimeClaudeRemoteDesc") : t("provider.runtimeClaudeDesc"),
      hint: t("provider.ccSwitchHint"),
    },
    {
      key: "codex" as const,
      name: "Codex",
      icon: <OpenAI size={18} />,
      description: isRemote ? t("provider.runtimeCodexRemoteDesc") : t("provider.runtimeCodexDesc"),
      hint: null,
    },
    {
      key: "gemini" as const,
      name: "Gemini CLI",
      icon: <Google size={18} />,
      description: isRemote ? t("provider.runtimeGeminiRemoteDesc") : t("provider.runtimeGeminiDesc"),
      hint: null,
    },
  ]).map((runtime) => ({
    ...runtime,
    status: runtimeStatus?.engines?.[runtime.key] ?? null,
  }));
    ...
    <div className="space-y-6">
      ...
      {!loading && (
        <div className="rounded-lg border border-border/50 p-4 space-y-2">
          ...
          {runtimeProviders.map((runtime, index) => {
            const isLastRuntime = index === runtimeProviders.length - 1;
            const showDivider = sorted.length > 0 || !isLastRuntime;
            const isSelected = activeEngine === runtime.key;
            const isReady = !!runtime.status?.ready;
            const statusText = runtime.status
              ? `${isReady ? t("connection.connected") : t("connection.disconnected")}${runtime.status.version ? ` · v${runtime.status.version}` : ""}`
            ...
            return (
              <div
                key={runtime.key}
                className={cn(
                  "rounded-md py-2.5 px-1 transition-colors",
                  isSelected ? "bg-accent/40" : "hover:bg-accent/20",
                  showDivider ? "border-b border-border/30" : ""
                )}
                onClick={() => handleEngineSelect(runtime.key)}
                role="button"
                tabIndex={0}
                onKeyDown={(event) => {
                  if (event.key === "Enter" || event.key === " ") {
                    event.preventDefault();
                    handleEngineSelect(runtime.key);
                  }
                }}
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {isSelected && selectedProviderLabel && (
                        <Badge variant="outline" className="text-[10px] px-1.5 py-0">
                          {selectedProviderLabel}
                        </Badge>
                      )}
                      {runtime.key === "claude" && Object.keys(envDetected).length > 0 && (
                    ...
                    {isSelected && selectedProviderLabel && (
                      <p className="text-[11px] text-muted-foreground leading-relaxed">
                        {t('provider.currentProvider', { name: selectedProviderLabel })}
                      </p>
                    )}
                    {runtime.hint && (
                      <p className="text-[11px] text-muted-foreground leading-relaxed">
