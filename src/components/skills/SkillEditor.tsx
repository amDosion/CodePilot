...
export function SkillEditor({ skill, onSave, onDelete }: SkillEditorProps) {
  ...
  const isTomlFile = skill.filePath.endsWith(".toml");
  ...
  const markdownContent = (
    isTomlFile ? (
      <pre className="h-full overflow-auto p-4 text-sm leading-6 whitespace-pre-wrap font-mono">
        {content}
      </pre>
    ) : (
      ...
      <div className="flex-1 min-h-0 overflow-hidden">
        {viewMode === "edit" && (
          <Textarea
            ...
            placeholder={isTomlFile ? 'Write your command definition in TOML...' : t('skills.placeholder')}
          />
        )}
        {viewMode === "preview" && (
