...
export const FileTreeFolder = ({
  ...
  const row = (
    <div className="group/folder flex w-full items-center gap-1 rounded px-2 py-1 text-left transition-colors hover:bg-muted/50">
          ...
          {tooltip ? (
            <Tooltip>
              ...
              <TooltipContent
                side="bottom"
                align="start"
                className="max-w-80 break-all font-mono text-[11px]"
              >
                {tooltip}
              </TooltipContent>
            </Tooltip>
          ...
          {(onAdd || actions) && (
            <FileTreeActions>
              {onAdd && (
                <button
                  type="button"
                  className="flex size-5 shrink-0 items-center justify-center rounded opacity-0 transition-opacity hover:bg-muted focus-visible:opacity-100 group-hover/file:opacity-100"
                  onClick={handleAdd}
                  title={addLabel}
                >
