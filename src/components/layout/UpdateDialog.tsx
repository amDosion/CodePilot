...
export function UpdateDialog() {
  ...
  const handleDontShowToday = () => {
    // Snooze until end of today (midnight)
    const tomorrow = new Date();
    tomorrow.setHours(23, 59, 59, 999);
    localStorage.setItem(UPDATE_SNOOZED_UNTIL_KEY, String(tomorrow.getTime()));
    dismissUpdate();
  };

  const handleNeverShow = () => {
    // Sync to both localStorage (for immediate effect) and DB (for persistence)
    localStorage.setItem('codepilot:update_dialog_enabled', 'false');
    fetch("/api/settings/app", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ settings: { update_dialog_enabled: 'false' } }),
    }).catch(() => {});
    dismissUpdate();
  };
        ...
        <DialogFooter className="flex-col gap-2 sm:flex-row sm:gap-0">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="start">
              <DropdownMenuItem onClick={dismissUpdate}>
                {t('update.later')}
              </DropdownMenuItem>
              <DropdownMenuItem onClick={handleDontShowToday}>
                {t('update.dontShowToday')}
              </DropdownMenuItem>
              <DropdownMenuItem onClick={handleNeverShow}>
                {t('update.neverShow')}
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
