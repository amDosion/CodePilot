...
export function SplitColumn({ sessionId, isActive, onClose, onFocus }: SplitColumnProps) {
  ...
  const { setWorkingDirectory, setSessionId, setSessionTitle: setPanelSessionTitle, setPanelOpen, setWorkspaceMode, setRemoteConnectionId } = usePanel();
    ...
    async function loadSession() {
      try {
        ...
        if (res.ok) {
          ...
          setSessionEngineType(data.session.engine_type || "claude");
          ...
          setSessionWorkspaceTransport(data.session.workspace_transport === 'ssh_direct' ? 'ssh_direct' : 'local');
  ...
  useEffect(() => {
    ...
    setWorkspaceMode(sessionWorkspaceTransport === 'ssh_direct' ? 'remote' : 'local');
    setRemoteConnectionId(sessionWorkspaceTransport === 'ssh_direct' ? sessionRemoteConnectionId : "");
    setSessionId(sessionId);
    setPanelOpen(true);
    if (sessionTitle) {
      setPanelSessionTitle(sessionTitle);
    }
  }, [isActive, sessionId, sessionRemoteConnectionId, sessionTitle, sessionWorkingDir, sessionWorkspaceTransport, setPanelOpen, setPanelSessionTitle, setRemoteConnectionId, setSessionId, setWorkingDirectory, setWorkspaceMode]);
