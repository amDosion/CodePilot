  ...
  const checkPrereqs = useCallback(async () => {
    ...
    try {
      ...
      if (result.hasClaude) {
        setLogs((prev) => [
          ...prev,
          t('install.nodeFound', { version: result.nodeVersion || "unknown" }),
          t('install.claudeFound', { version: result.claudeVersion || "unknown" }),
        ]);
      ...
      if (result.hasNode) {
        setLogs((prev) => [
          ...prev,
          t('install.nodeFound', { version: result.nodeVersion || "unknown" }),
          t('install.claudeMissing'),
        ]);
      } else {
        setLogs((prev) => [
          ...prev,
          t('install.nodeMissing'),
          t('install.claudeMissing'),
        ]);
      }
      setPhase("confirm");
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            {phase === "confirm"
              ? t('install.confirmDetectedMissing')
              : t('install.confirmAutoInstall')}
          </DialogDescription>
        </DialogHeader>
              ...
              <div className="rounded-lg bg-amber-500/10 px-4 py-3 text-sm space-y-1.5">
                {prereqs && !prereqs.hasNode && (
                  <p className="text-amber-700 dark:text-amber-400">
                    {t('install.nodeMissing')} ({prereqs.platform === "win32" ? "winget" : "Homebrew"})
                  </p>
                )}
                {prereqs?.hasNode && (
                  <p className="text-emerald-700 dark:text-emerald-400">
                    {t('install.nodeFoundStatus', { version: prereqs.nodeVersion || "unknown" })}
                  </p>
                )}
                <p className="text-amber-700 dark:text-amber-400">
                  {t('install.claudeMissingStatus')}
                </p>
              </div>
                <p className="text-sm text-muted-foreground">
                {t('install.installPrompt', {
                  target: prereqs && !prereqs.hasNode
                    ? t('install.targetNodeAndClaudeCli')
                    : t('install.targetClaudeCli'),
                })}
              </p>
            </div>
          )}
