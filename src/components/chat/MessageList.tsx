...
export function MessageList({
  ...
  const normalizedEngine = (engineType || '').trim().toLowerCase();
  const isCodex = normalizedEngine === 'codex';
  const isGemini = normalizedEngine === 'gemini';
  ...
  if (messages.length === 0 && !isStreaming) {
    return (
      <div className="flex flex-1 items-center justify-center">
        <ConversationEmptyState
          title={isCodex ? t('messageList.codexChat') : isGemini ? t('messageList.geminiChat') : t('messageList.claudeChat')}
