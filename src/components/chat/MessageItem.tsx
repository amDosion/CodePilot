...
import { CommandSelectBlock, type CommandSelectData } from './CommandSelectBlock';
...
function parseCommandSelect(text: string): { beforeText: string; data: CommandSelectData; afterText: string } | null {
  const regex = /```command-select\s*\n?([\s\S]*?)\n?\s*```/;
  const match = text.match(regex);
  if (!match) return null;
  try {
    const json = JSON.parse(match[1]);
    ...
    return {
      beforeText,
      data: json as CommandSelectData,
      afterText,
    };
  } catch {
    return null;
...
export const MessageItem = memo(function MessageItem({ message }: MessageItemProps) {
  ...
  const isImageGenNotice = isUser && message.content.startsWith('[__IMAGE_GEN_NOTICE__');
...
const AssistantContent = memo(function AssistantContent({ displayText, messageId }: { displayText: string; messageId: string }) {
  return useMemo(() => {
    ...
    // Interactive command selection (e.g. /model, /permissions)
    const cmdSelect = parseCommandSelect(displayText);
    if (cmdSelect) {
      return (
        <>
          {cmdSelect.beforeText && <MessageResponse>{cmdSelect.beforeText}</MessageResponse>}
          <CommandSelectBlock data={cmdSelect.data} />
          {cmdSelect.afterText && <MessageResponse>{cmdSelect.afterText}</MessageResponse>}
        </>
      );
    }
