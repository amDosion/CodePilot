...
export function GalleryDetail({
  ...
  const [currentImageIndexByItem, setCurrentImageIndexByItem] = useState<Record<string, number>>({});
  const [confirmDeleteByItem, setConfirmDeleteByItem] = useState<Record<string, boolean>>({});

  const handleDownload = useCallback(async () => {
    if (!item) return;
    const currentImageIndex = currentImageIndexByItem[item.id] ?? 0;
  ...
  }, [item, currentImageIndexByItem]);

  const handleDelete = useCallback(() => {
    if (!item) return;
    const confirmDelete = confirmDeleteByItem[item.id] ?? false;
    if (!confirmDelete) {
      setConfirmDeleteByItem((prev) => ({ ...prev, [item.id]: true }));
      return;
    }
    onDelete?.(item.id);
    onOpenChange(false);
    setConfirmDeleteByItem((prev) => ({ ...prev, [item.id]: false }));
  }, [item, confirmDeleteByItem, onDelete, onOpenChange]);
            ...
            {hasMultipleImages && (
              <>
                <button
                  type="button"
                  onClick={() => {
                    setCurrentImageIndexByItem((prev) => {
                      const current = prev[item.id] ?? 0;
                      const next = current > 0 ? current - 1 : item.images.length - 1;
                ...
                <button
                  ...
                  onClick={() => {
                    setCurrentImageIndexByItem((prev) => {
                      const current = prev[item.id] ?? 0;
                      const next = current < item.images.length - 1 ? current + 1 : 0;
                      return { ...prev, [item.id]: next };
                    });
                  }}
