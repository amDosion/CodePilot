...
export type WorkspaceTransport = 'local' | 'ssh_direct';
export type RemoteShellState = 'idle' | 'starting' | 'running' | 'stopped' | 'error';
...
export interface ChatSession {
  ...
  engine_type: string; // Engine id, default 'claude'
  engine_session_id: string; // Engine-native session/thread id
  project_name: string;
...
export interface GitFileStatus {
  ...
  index: string;
  working_dir: string;
  staging?: 'staged' | 'unstaged' | 'partial';
}
...
export interface GitTargetDescriptor {
  mode: 'local' | 'remote';
  cwd: string;
  session_id?: string;
  connection_id?: string;
...
export interface GitHubCloneProgressEvent {
  message: string;
  phase: 'preparing' | 'cloning' | 'done' | 'error';
  percent?: number | null;
  done?: boolean;
  error?: string;
}
...
// --- Native command controller API (frontend contract) ---

export interface NativeCommandControllerRequestContext {
  mode?: string;
  model?: string;
  provider_id?: string;
  reasoning_effort?: string;
...
export interface NativeCommandControllerRequest {
  [key: string]: unknown;
  session_id?: string;
  engine_type: string;
  command: string;
...
export interface NativeCommandStatePatch {
  model?: string;
  provider_id?: string;
  reasoning_effort?: string;
  mode?: string;
...
export interface NativeCommandControllerResponse {
  handled: boolean;
  message?: string;
  state_patch?: NativeCommandStatePatch;
  data?: unknown;
  error?: string;
}
...
export interface ShellTranscriptResponse {
...
export interface GitStatusActionResponse extends GitActionResponse {
  status?: GitStatus;
}
...
export interface GitCloneResponse extends GitActionResponse {
  destination?: string;
}

export interface GitCommitMessageResponse {
  suggestion: GitCommitMessageSuggestion;
  is_repo: boolean;
  target: GitTargetDescriptor;
}

export interface GitHubDeviceCodeStatusResponse {
  enabled: boolean;
  configured: boolean;
...
export interface GitHubDeviceCodeResponse extends GitHubDeviceCodeStatusResponse {
  flow?: GitHubDeviceCode;
}

export interface GitHubDevicePollResponse {
  status: 'pending' | 'slow_down' | 'approved' | 'denied' | 'expired' | 'error';
  interval?: number;
