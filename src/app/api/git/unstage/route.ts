...
import { describeGitTarget, gitStatus, gitUnstage, isGitServiceError, parseGitContextFromBody, resolveGitTarget } from '@/lib/git';
...
export async function POST(request: NextRequest) {
  try {
    ...
    await gitUnstage(target, parseFiles(body.files));
    ...
    return NextResponse.json<ErrorResponse>(
      { error: error instanceof Error ? error.message : 'Failed to unstage Git changes' },
      { status: 500 },
    );
  }
}
