...
import { describeGitTarget, gitFetch, gitStatus, isGitServiceError, parseGitContextFromBody, readGitString, resolveGitTarget } from '@/lib/git';
...
export async function POST(request: NextRequest) {
  try {
    ...
    await gitFetch(target, readGitString(body.remote));
    ...
    return NextResponse.json<ErrorResponse>(
      { error: error instanceof Error ? error.message : 'Failed to fetch Git changes' },
      { status: 500 },
    );
  }
}
