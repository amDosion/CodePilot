...
import {
  cleanupEmptyGeminiCommandDirs,
  findGeminiCommandFile,
...
function findStructuredSkillByName(dir: string, name: string, source: SkillSource): SkillMatch | null {
  if (!fs.existsSync(dir)) return null;
  const directPath = path.join(dir, name, "SKILL.md");
  if (fs.existsSync(directPath)) {
    return { filePath: directPath, source };
  }
...
function findCodexSkillFile(name: string, cwd?: string): SkillMatch | null {
  const projectMatch = findStructuredSkillByName(getCodexProjectSkillsDir(cwd), name, "project");
  if (projectMatch) return projectMatch;

  return findStructuredSkillByName(getCodexGlobalSkillsDir(), name, "global");
}
...
export async function GET(
  ...
  try {
    ...
    const engineType = normalizeEngineType(url.searchParams.get("engine_type"));
    ...
    if (engineType === "codex") {
      if (sourceParam) {
        return NextResponse.json(
          { error: "source is not supported for codex skills" },
          { status: 400 }
        );
      }

      const found = findCodexSkillFile(name, cwdParam);
      if (!found) {
    ...
    if (engineType === "gemini") {
      const found = findGeminiCommandFile(name, cwdParam);
      if (!found) {
...
export async function PUT(
  ...
  try {
    ...
    if (engineType === "codex") {
      ...
      const meta = parseSkillFrontMatter(updated);
    ...
    if (engineType === "gemini") {
      ...
      fs.writeFileSync(found.filePath, content ?? "", "utf-8");
      const updated = content ?? "";
      const meta = parseGeminiCommandContent(updated, name);
...
export async function DELETE(
  ...
  try {
    ...
    if (engineType === "gemini") {
      ...
      cleanupEmptyGeminiCommandDirs(found.filePath, cwdParam);
      return NextResponse.json({ success: true });
    }
